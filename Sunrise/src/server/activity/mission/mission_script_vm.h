 ...
 * @param spawnSetHash Optional receiver for the declared `spawn_set_hash`, left untouched when the
 *                     script declares none. It is the authored spawn set the launch arrives at.
 */
[[nodiscard]] std::int32_t probe_initial_state_region(std::span<const char> source,
                                                      std::string_view sdkLuaSearchPath,
                                                      std::uint32_t* spawnSetHash = nullptr) noexcept;
